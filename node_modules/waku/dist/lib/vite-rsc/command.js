...
async function loadConfig() {
    ...
    if (existsSync('waku.config.ts') || existsSync('waku.config.js')) {
        const imported = await vite.runnerImport('/waku.config');
        config = imported.module.default;
    }
    return resolveConfig(config);
}
async function startDevServer(host, port, config) {
    const server = await vite.createServer({
        configFile: false,
        plugins: [
            combinedPlugins(config)
        ],
        server: host ? {
            host,
            port
    ...
    async function handleConfigChange(changedFile) {
        const dirname = path.dirname(changedFile);
        const filename = path.basename(changedFile);
        if (dirname === process.cwd() && (filename === 'waku.config.ts' || filename === 'waku.config.js')) {
...
export async function runCommand(cmd, flags) {
    ...
    if (process.env.NODE_ENV && process.env.NODE_ENV !== nodeEnv) {
        console.warn(`Warning: NODE_ENV is set to '${process.env.NODE_ENV}', but overriding it to '${nodeEnv}'.`);
    }
    ...
    if (cmd === 'dev') {
        ...
        const serveFileUrl = pathToFileURL(path.resolve(distDir, 'serve-node.js')).href;
        if (host) {
    ...
    for(let port = startPort;; port++){
        try {
            await new Promise((resolve, reject)=>{
                const srv = net.createServer().once('error', reject).once('listening', ()=>srv.close(()=>resolve())).listen(port);
            });
            return port;
        } catch (err) {
