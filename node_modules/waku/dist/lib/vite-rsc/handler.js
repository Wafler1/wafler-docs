import { createFromReadableStream, createTemporaryReferenceSet, decodeAction, decodeFormState, decodeReply, loadServerAction, renderToReadableStream } from '@vitejs/plugin-rsc/rsc';
...
const toProcessBuild = (handleBuild)=>async ({ emitFile })=>{
        const renderUtils = createRenderUtils(undefined, renderToReadableStream, createFromReadableStream, loadSsrEntryModule);
        let fallbackHtml;
        ...
        await handleBuild({
            ...
            generateDefaultHtml: async (fileName)=>{
                await emitFile(joinPath(DIST_PUBLIC, fileName), await getFallbackHtml());
            }
        });
        await emitFile(joinPath(DIST_SERVER, BUILD_METADATA_FILE), `export const buildMetadata = new Map(${JSON.stringify(Array.from(buildMetadata))});`);
    };
export const createServerEntryAdapter = (fn)=>(handlers, options)=>{
