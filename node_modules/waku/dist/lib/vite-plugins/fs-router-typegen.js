...
export function toIdentifier(input) {
    ...
    return 'File_' + identifier.split('_').map((part)=>{
        if (part[0] === undefined) {
            return '';
        }
...
const parseModule = async (filePath)=>{
    ...
    const loader = filePath.endsWith('.tsx') ? 'tsx' : filePath.endsWith('.ts') ? 'ts' : 'jsx';
    const transformed = await transformWithEsbuild(source, filePath, {
        loader,
        jsx: 'preserve'
    });
    return parseAstAsync(transformed.code, {
        jsx: true
    });
};
...
export const fsRouterTypegenPlugin = (opts)=>{
    return {
        ...
        configureServer (server) {
            ...
            const updateGeneratedFile = async (file)=>{
...
export async function detectFsRouterUsage(srcDir) {
    const existingServerEntry = EXTENSIONS.map((ext)=>joinPath(srcDir, SRC_SERVER_ENTRY + ext)).find((entriesFile)=>existsSync(entriesFile));
    ...
    try {
        ...
        const usesFsRouter = file.body.some((node)=>{
            ...
            if (node.source.type !== 'Literal' || typeof node.source.value !== 'string' || !node.source.value.startsWith('waku')) {
                return false;
            }
...
export async function generateFsRouterTypes(pagesDir) {
    ...
    const fileExportsGetConfig = async (filePath)=>{
        ...
        return file.body.some((node)=>{
            ...
            if (node.declaration?.type === 'VariableDeclaration' && node.declaration.declarations.some((decl)=>decl.id.type === 'Identifier' && decl.id.name === 'getConfig')) {
                return true;
            }
            if (node.declaration?.type === 'FunctionDeclaration' && node.declaration.id?.name === 'getConfig') {
                return true;
            }
            return node.specifiers.some((specifier)=>specifier.type === 'ExportSpecifier' && getExportedName(specifier) === 'getConfig');
        });
    };
    const generateFile = async (filePaths)=>{
        const fileInfo = [];
        ...
        const lines = [
            ...
            '// prettier-ignore',
            hasAnyGetConfig ? "import type { PathsForPages, GetConfigResponse } from 'waku/router';" : "import type { PathsForPages } from 'waku/router';"
        ];
        if (hasAnyGetConfig) {
            lines.push('');
            for (const file of fileInfo){
                ...
                lines.push(`// prettier-ignore`, `import type { getConfig as ${moduleName}_getConfig } from './${SRC_PAGES}/${file.src.replace('.tsx', '')}';`);
            }
        }
        ...
        for (const file of fileInfo){
            const moduleName = moduleNames[file.src];
            if (file.hasGetConfig) {
                lines.push(`| ({ path: '${file.path}' } & GetConfigResponse<typeof ${moduleName}_getConfig>)`);
            } else {
                lines.push(`| { path: '${file.path}'; render: 'static' }`);
            }
        }
        lines[lines.length - 1] += ';';
        lines.push('', '// prettier-ignore', "declare module 'waku/router' {", '  interface RouteConfig {', '    paths: PathsForPages<Page>;', '  }', '  interface CreatePagesConfig {', '    pages: Page;', '  }', '}', '');
...
//# sourceMappingURL=fs-router-typegen.js.map